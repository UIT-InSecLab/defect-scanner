...
class GadgetVectorizer:
    ...
    def __init__(self, embedding, model_name, vector_length, device):
    ...
    def vectorize(self, gadget):
        ...
        if self.emb == 'word2vec':
            ...
            with torch.no_grad():
                ...
                embeddings = outputs[0][0][:50]
                embeddings = F.interpolate(embeddings.unsqueeze(0), size=50, mode='linear').squeeze(0)
            vectors = np.zeros(shape=(50, self.vector_length))
