...
"""
parser = argparse.ArgumentParser(prog='python3 codegadget_main.py', description='NLP-based models for vulnerability detection using code gadget')
...
parser.add_argument('-dt', help='Input file consisting code gadget data for training and evaluating', dest='dt')
parser.add_argument('-df', help='Input file consisting vectors converted from code gadgets', dest='df')
parser.add_argument('-emb', help='Embedding technique to covert code gadgets to vector. Supported: codebert, word2vec. Required when using raw SeVC or code gadget file, not required for processing vectors.', dest='emb') 
...
def get_vectors_df(filename, embedding, modelname, vector_length):
    ...
    print("[+] Vectorizing code gadgets completed.")
    return df
...
def main():
    ...
    if args.df != None:
        ...
        print("[+] Code gadget file provided: " + args.dt + ". Try reading the file...")
        filename = args.dt
